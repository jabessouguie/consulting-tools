...
# 🧪 Script de Validation Automatique - WEnvision Consulting Tools
# Usage: ./validate.sh

set -e  # Exit on error

echo "═══════════════════════════════════════════════════"
echo "🔍 VALIDATION WENVISION CONSULTING TOOLS"
echo "═══════════════════════════════════════════════════"
echo ""
...
# Check WEnvision palette in pdf_converter.py
if grep -q "#E86F51" utils/pdf_converter.py; then
    echo -e "  ${GREEN}✅${NC} WEnvision Corail color in pdf_converter.py"
fi

if grep -q "#F5E6E8" utils/pdf_converter.py; then
    echo -e "  ${GREEN}✅${NC} WEnvision Rose Poudré color in pdf_converter.py"
fi

if grep -q "#C4624F" utils/pdf_converter.py; then
    echo -e "  ${GREEN}✅${NC} WEnvision Terracotta color in pdf_converter.py"
fi
...
echo -e "${GREEN}✅ PDF Color Fix${NC}        : Palette WEnvision préservée"
