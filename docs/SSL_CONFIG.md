...
0 2 * * * certbot renew --quiet && systemctl restart wenvision-agents
```
...
2. Configuration Nginx (`/etc/nginx/sites-available/wenvision`) :
...
```bash
sudo ln -s /etc/nginx/sites-available/wenvision /etc/nginx/sites-enabled/
sudo nginx -t
