...
from utils.consultant_profile import ConsultantProfile  # noqa: E402
...
class ArticleGeneratorAgent:
    ...
    def generate_article(
        self, idea: str, target_length: str = "medium", use_context: bool = False
    ) -> str:
        ...
        Args:
            ...
            use_context: Utiliser le contexte enrichi (articles precedents,
                        veille, personnalite)
        """
...
Tu rediges des articles de blog techniques et accessibles sur l'IA, \
la data et la transformation digitale.
...
- Critique constructif : tu poses les bonnes questions, tu challenges les \
idees recues
- Pedagogue : tu expliques les concepts pas a pas
- Engageant : tu interpelles le lecteur, tu poses des questions rhetoriques
- INTERDICTION GRAMMATICALE : n'utilise absolument jamais le pronom "on" \
(privilegie "nous", "vous" ou la voix passive)
- REGLE TYPOGRAPHIQUE : ne mets jamais de majuscule au premier mot qui \
suit les deux-points (:)
...
cluster: "[sous-theme, ex: ia-entreprise-roi, data-governance, \
cloud-migration]"
...
> [Genere ici un prompt detaille pour creer une illustration premium et \
cinematique adaptee au theme de l article. Style : Unreal Engine 5, \
corporate tech, palette bleu froid + ambre/or, format 1792x1024 (paysage)]
>
> **Dimensions recommandees** : 1792x1024px (format paysage pour blog header)

3. STRUCTURE ARTICLE (CONCLUSION INVERSEE) :
- Titre accrocheur (H1)
- Chapeau et elements de conclusion : livre d emblee les conclusions de \
l article et les messages cles a retenir en 2 ou 3 phrases
...
- Pas d'emojis"""
        ...
        prompt = f"""Redige un article de blog complet ({target_words}) \
...
IMPORTANT : Commence l article par les metadonnees YAML, puis l image \
placeholder **![][image1]**, puis le contenu.
...
Retourne l article complet avec metadonnees en markdown, sans preambule."""
    ...
    def generate_linkedin_post(
        self, article: str, article_title: str = "", article_url: str = ""
    ) -> str:
        """Genere un post LinkedIn pour promouvoir l'article"""
        print("  [2/4] Generation du post LinkedIn...")

        system_prompt = f"""# CONTEXTE
Tu viens de publier un article de blog sur le site de \
{self.consultant_info['company']}. Tu dois en faire la promotion via \
un post sur LinkedIn.

# ROLE
Agis en tant que "ghostwriter" personnel, specialise dans la creation \
de contenu "thought leadership" pour des consultants en transformation \
digitale sur LinkedIn. Tu connais parfaitement les codes de la plateforme.

# PROFIL
- Poste : {self.consultant_info['title']} chez \
{self.consultant_info['company']}.
- Style : Pragmatique, gen z, parisien, oriente resultats, avec une vision \
critique et constructive sur les technologies. Aime aller a contre-courant \
des "buzzwords".

# OBJECTIFS DU POST
1. Susciter le debat et l'engagement (commentaires, partages).
2. Asseoir l'expertise en montrant une perspective nuancee et basee sur \
...
1. Accroche percutante : Commence par une phrase ou une question un peu \
provocatrice qui remet en cause une idee recue.
...
#GouvernanceDeDonnees, #TransformationDigitale, #WEnvision).
...
Genere le post LinkedIn."""
    ...
    def research_web_sources(self, article: str) -> List[Dict[str, str]]:
        """Recherche des sources web fiables."""
        print("  [4/4] Recherche de sources web...")

        system_prompt = (
            """Tu es un assistant de recherche specialise """
            """dans la data et l'IA.
"""
            """Tu dois trouver des sources fiables pour """
            """etayer les points d'un article."""
        )

        prompt = f"""A partir de cet article, identifie les 5-8 \
affirmations ou concepts cles qui gagneraient a etre sourcess :

{article[:3000]}

Pour chaque affirmation, suggere une source fiable \
(etude, rapport, article de reference) avec :
- Le titre de la source
- L'URL probable (utilise des sources connues : Gartner, McKinsey, \
Harvard Business Review, MIT Technology Review, etc.)
...
```"""
    ...
    def generate_illustration_prompt(self, article: str) -> str:
        ...
        system_prompt = (
            """Tu es un expert en generation de prompts """
            """pour DALL-E et Midjourney.\n"""
            """Ton role est de creer un prompt concis et """
            """efficace pour generer une illustration premium."""
        )

        user_prompt = f"""Extrait de l article :
{article[:1500]}

Genere un prompt DALL-E/Midjourney (max 300 caracteres) pour une \
illustration premium de blog tech.

CONTRAINTES :
- Style : Unreal Engine 5 render, corporate tech aesthetic, \
isometric or wide-angle
...
- Contenu : Represente le concept central de l article de maniere \
metaphorique (PAS literal)
...
Retourne UNIQUEMENT le prompt, sans explication."""
        ...
        try:
            ...
            return (
                "A premium cinematic illustration for a tech "
                "consulting blog post. Unreal Engine 5 render, "
                "isometric view, 8k quality. Corporate tech "
                "aesthetic with cool electric blues and warm "
                "amber/gold accents. Sophisticated, futuristic "
                "but grounded mood. Wide landscape format, 16:9."
            )

    def run(
        self, idea: str, target_length: str = "medium", use_context: bool = False
    ) -> Dict[str, Any]:
        """Pipeline complet : article + post + image + sources

        Args:
            idea: Sujet de l article
            target_length: short, medium ou long
            use_context: Utiliser le contexte enrichi
                (articles precedents, veille, personnalite)
        """
        print("\n  GENERATION D'ARTICLE")
        print(f"  Idee : {idea[:100]}...")
        if use_context:
            print("  Mode : Contexte enrichi active \U0001f3af")
        ...
        print(f"  Article sauvegarde : " f"{article_path.relative_to(self.base_dir)}")
